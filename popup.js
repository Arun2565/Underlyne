...
async function renderHighlights() {
  ...
  // Sort by article (DOM) order
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    const orderResult = await chrome.tabs.sendMessage(tab.id, { action: 'getHighlightsOrder' });
    if (orderResult && Array.isArray(orderResult.order)) {
      const orderMap = {};
      orderResult.order.forEach((id, idx) => { orderMap[id] = idx; });
      highlights.sort((a, b) => {
    ...
    highlights.sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));
  }
...
<div class="footer">Exported on ${new Date().toLocaleString()} &middot; ${result.annotations.length} annotation${result.annotations.length !== 1 ? 's' : ''}</div>
  ...
  document.getElementById('btn-markdown').addEventListener('click', async () => {
    ...
    const typeLabels = { highlight: 'Highlight', underline: 'Underline' };
    const colorNames = { yellow: 'Yellow', green: 'Green', blue: 'Blue', red: 'Red', purple: 'Purple' };
    const lines = [];
    lines.push(`# ${tab.title || 'Underlyne Annotations'}`);
    lines.push('');
    lines.push(`Source: ${currentUrl}`);
    lines.push(`Exported: ${new Date().toLocaleString()}`);
    lines.push('');
    lines.push('## Annotations');
    lines.push('');
    for (const hl of highlights) {
      const typeLabel = typeLabels[hl.type] || hl.type;
      const colorLabel = colorNames[hl.color] || hl.color;
      const text = hl.text || '';
      lines.push(`- **"${text}"** — ${typeLabel} (${colorLabel})`);
    }
    lines.push('');
    lines.push('---');
    lines.push(`*${highlights.length} annotation${highlights.length !== 1 ? 's' : ''}*`);
    const md = lines.join('\n');
    const blob = new Blob([md], { type: 'text/markdown' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'underlyne-annotations-' + Date.now() + '.md';
    a.click();
    URL.revokeObjectURL(url);
  });
